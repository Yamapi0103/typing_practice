<template>
  <div class="select-none">
    <div v-for="(row, ri) in KEYBOARD_ROWS" :key="ri" class="flex gap-1 mb-1 justify-center">
      <div v-if="ri === 1" class="w-16" />
      <div v-if="ri === 2" class="w-20" />
      <div v-if="ri === 3" class="w-28" />
      ...
      >
        <span class="text-[10px] leading-none font-bold opacity-60">{{ keyDef.display }}</span>
        <span v-if="keyDef.bopomofo" class="text-sm leading-none mt-0.5">{{ keyDef.bopomofo }}</span>
    ...
    <div v-if="showLegend" class="flex flex-wrap gap-2 mt-4 justify-center">
      ...
      >
        <span class="w-3 h-3 rounded-sm inline-block" :style="{ backgroundColor: FINGER_COLORS[zone] }" />
...
function handlePress(key: string) {
  ...
  setTimeout(() => { pressedKey.value = null }, 120)
