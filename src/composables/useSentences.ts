...
async function fetchZh(): Promise<void> {
  ...
  try {
    ...
    if (parsed.length > 0) {
      ...
      zhFetched = true
      Object.keys(queues).filter(k => k.startsWith('zh')).forEach(k => delete queues[k])
    }
  } catch {
...
async function fetchEn(): Promise<void> {
  ...
  try {
    ...
    if (parsed.length > 0) {
      ...
      Object.keys(queues).filter(k => k.startsWith('en')).forEach(k => delete queues[k])
    }
  } catch {
...
function getRandomSentence(level: Level | null = null): Sentence {
  return getQueue(level).pop() ?? SENTENCES[Math.floor(Math.random() * SENTENCES.length)]
}
