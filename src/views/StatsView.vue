  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-3 gap-4">
      <div class="bg-gray-900 rounded-xl p-4 text-center">
        <div class="text-3xl font-bold text-indigo-400">{{ store.records.length }}</div>
    ...
    <div class="bg-gray-900 rounded-xl p-4">
      ...
      <div v-if="store.records.length === 0" class="text-gray-500 text-sm text-center py-4">
        尚無練習紀錄，快去打字吧！
      </div>
      <div v-else class="space-y-2 max-h-96 overflow-y-auto">
        ...
        >
          <span class="text-gray-400 text-xs w-28 flex-shrink-0">{{ formatDate(r.date) }}</span>
          <span class="text-indigo-300 font-bold w-16 text-right">{{ r.wpm }} WPM</span>
          <span class="w-16 text-right" :class="r.accuracy >= 95 ? 'text-green-400' : r.accuracy >= 80 ? 'text-yellow-400' : 'text-red-400'">
            {{ r.accuracy }}%
          </span>
