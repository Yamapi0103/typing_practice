<template>
  <div class="max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold text-center mb-2 text-indigo-300">
      鍵盤指法大全
    </h1>
    ...
    <div class="mt-3 h-8 flex items-center justify-center gap-3 text-sm">
      <template v-if="activeKey === ' '">
        <span class="font-mono text-white bg-gray-700 px-2 py-0.5 rounded"
          >Space</span
        >
        ...
        <span
          ...
          :style="{
            backgroundColor: FINGER_COLORS['L5'] + '33',
            color: FINGER_COLORS['L5'],
          }"
        >
      ...
      <template v-else-if="activeKeyDef">
        <span class="font-mono text-white bg-gray-700 px-2 py-0.5 rounded">{{
          activeKeyDef.display
        }}</span>
        <span
          v-if="activeKeyDef.bopomofo"
          class="text-indigo-300 text-lg font-bold"
          >{{ activeKeyDef.bopomofo }}</span
        >
        <span class="text-gray-400">·</span>
        <span
          class="text-xs px-2 py-0.5 rounded"
          :style="{
            backgroundColor: FINGER_COLORS[activeKeyDef.finger] + '33',
            color: FINGER_COLORS[activeKeyDef.finger],
          }"
        >
        ...
        >
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1 mt-1">
              ...
              >
                {{ k.display
                }}<span v-if="k.bopomofo" class="text-indigo-300 ml-0.5">{{
                  k.bopomofo
                }}</span>
              </span>
            </div>
    ...
    <div
      class="mt-6 bg-indigo-950 border border-indigo-800 rounded-xl p-4 text-sm text-gray-300 space-y-1"
    >
